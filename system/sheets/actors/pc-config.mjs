...
export default class PcConfig extends ActorDocumentSheet {
  ...
  static get defaultOptions() {
    return foundry.utils.mergeObject(super.defaultOptions, {
      ...
      tabs: [{navSelector: ".tabs", contentSelector: ".tabs-container", initial: "character"}],
      closeOnSubmit: false,
      submitOnChange: true,
      submitOnClose: true
    });
  ...
  async getData() {
    ...
    context.PROFICIENCY_TYPES = this.getDatalistOptions(
      CONFIG.SYSTEM.PROFICIENCY_TYPES,
      this.object.system.proficiencies
    );
    ...
    context.SAVE_ADVANTAGE_TYPES = this.getDatalistOptions(
      CONFIG.SYSTEM.DAMAGE_TYPES,
      this.object.system.saveAdvantages
    );
    context.nextLevelXP = CONFIG.SYSTEM.XP_TABLE.get(this.object.system.level + 1);
    context.document.system.talents = Array.from(
      context.document._source.system.talents.map(talent => CONFIG.SYSTEM.TALENT_DOCUMENTS.get(talent))
    );
